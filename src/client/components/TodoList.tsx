...
export const TodoList: FC<TodoListProps> = ({ status }) => {
  ...
  const updateTodoStatus = trpc.todoStatus.update.useMutation()
  ...
  const handleUpdateTodoStatus = async (
    ...
    try {
      await updateTodoStatus.mutateAsync({ todoId, status: newStatus })
      refetch()
          ...
          <li key={todo.id}>
            ...
            >
              <div className="flex items-center gap-3">
                <Checkbox.Root
                  onClick={() => handleUpdateTodoStatus(todo.id, todo.status)}
                ...
                <label
                  className={`block pl-3 font-inter text-base font-medium text-gray-700 ${
              ...
              >
                <XMarkIcon className="h-full w-full fill-gray-700 object-cover" />
              </div>
            </div>
          </li>
        ))}
